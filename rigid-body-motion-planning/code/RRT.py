...
class RRT(SamplingMethod):
    ...
    def get_neighbors(self, new_node):
        ...
        """
        # Using kdtree to find the neighbors within neighbor size
        neighbor_indices = self.kdtree.query_ball_point(new_node.config, self.kdtree_d)
        # Retrieving the actual neighbor nodes using the indices
        neighbors = [self.samples[ii] for ii in neighbor_indices if self.samples[ii] != new_node]
    ...
    def rewire(self, new_node, neighbors):
        ...
        """
        # Return immediately if there are no neighbors
        if not neighbors:
            return
        
        # Computing the distances from the new node to each of its neighbors
        distances = [self.robot.distance(neighbor.config, new_node.config) for neighbor in neighbors]
        
        # Computing the total cost of each path from the start node to the neighbors through their current parents
        costs = []
        for ii in range(len(distances)):
            path, neighbor_cost = self.get_path_and_cost(self.start_node, neighbors[ii])
            costs.append(distances[ii] + neighbor_cost)
        
        # Sort the neighbors by the total path cost in ascending order
        idxs = np.argsort(np.array(costs))
        
        # Connect the new node to the neighbor with the least total path cost, provided there is no collision
        for idx in idxs:
            if not self.check_collision(new_node.config, neighbors[idx].config):
                new_node.parent = neighbors[idx]
                new_node.cost = distances[idx]
                break
        
        # For each neighbor, check if the path through the new node is less costly, and update if necessary
        path, new_cost = self.get_path_and_cost(self.start_node, new_node)
        for i, node in enumerate(neighbors):
            new_cost += distances[i]
            path, cost = self.get_path_and_cost(self.start_node, node)
            if cost > new_cost and not self.check_collision(node.config, new_node.config):
    ...
    def Informed_RRT_star(self, max_iterations=10000):
